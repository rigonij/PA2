...
$messages = $messages ?? [];
$withID = isset($withID) ? (int)$withID : 0;
$withName = $withName ?? "";
?>
                        ...
                        <div class="list-group">
                            ...
                            <?php else: ?>
                                <?php foreach ($conversations as $c): ?>
                                    <?php $isActive = ((int)($c["other_id"] ?? 0) === $withID); ?>
                                    <a href="messagerie.php?with=<?= (int)($c['other_id'] ?? 0) ?>"
                                        class="list-group-item list-group-item-action <?= $isActive ? 'active' : '' ?>">
                                        <strong><?= htmlspecialchars($c["other_name"] ?? "Inconnu") ?></strong>
                                        <?php if (!empty($c["unread_count"]) && (int)$c["unread_count"] > 0): ?>
                                            <span class="badge text-bg-danger float-end"><?= (int)$c["unread_count"] ?></span>
                                        <?php endif; ?>
                                        <br>
                                    ...
                                    <?php if (!empty($msg["is_mine"])): ?>
                                        <div class="mb-2 text-end">
                                            ...
                                            <div class="text-muted" style="font-size:0.7em">
                                                <?php $dt = new DateTime($msg["created_at"] ?? "now");
                                                echo $dt->format("d/m H:i"); ?>
                                            </div>
                                        </div>
                                    <?php else: ?>
