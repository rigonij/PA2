...
func main() {
	...
	mux.HandleFunc("/api/admin/stats", middleware.CORS(routes.AdminGetStats(database)))
	...
	mux.HandleFunc("/api/admin/events/", middleware.CORS(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPut {
			routes.AdminValidateEvent(database)(w, r)
	...
	mux.HandleFunc("/api/admin/payments", middleware.CORS(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet {
			routes.AdminGetPayments(database)(w, r)
		} else {
			w.WriteHeader(http.StatusMethodNotAllowed)
		}
	}))
	...
	mux.HandleFunc("/api/provider/services-submit/", middleware.CORS(routes.ProviderSubmitService(database)))

	mux.HandleFunc("/api/admin/interventions", middleware.CORS(routes.GetAdminInterventions(database)))
	mux.HandleFunc("/api/admin/interventions/status", middleware.CORS(routes.UpdateAdminInterventionStatus(database)))
